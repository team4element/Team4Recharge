...
public class DriveMotionPlanner implements CSVWritable {
    ...
    public DriveMotionPlanner() {
        final DCMotorTransmission transmission = new DCMotorTransmission(
                1.0 / DriveConstants.kDriveKv,
                Units.inches_to_meters(DriveConstants.kDriveWheelRadiusInches) * Units.inches_to_meters(DriveConstants
                        .kDriveWheelRadiusInches) * DriveConstants.kRobotLinearInertia / (2.0 * DriveConstants.kDriveKa),
                DriveConstants.kDriveVIntercept);
        mModel = new DifferentialDrive(
                DriveConstants.kRobotLinearInertia,
                DriveConstants.kRobotAngularInertia,
                DriveConstants.kRobotAngularDrag,
                Units.inches_to_meters(DriveConstants.kDriveWheelRadiusInches),
                Units.inches_to_meters(DriveConstants.kDriveWheelRadiusInches * DriveConstants.kTrackScrubFactor),
    ...
    protected Output updatePurePursuit(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
        double lookahead_time = DriveConstants.kPathLookaheadTime;
        ...
        while (actual_lookahead_distance < DriveConstants.kPathMinLookaheadDistance &&
        ...
        if (actual_lookahead_distance < DriveConstants.kPathMinLookaheadDistance) {
            lookahead_state = new TimedState<>(new Pose2dWithCurvature(lookahead_state.state()
                    .getPose().transformBy(Pose2d.fromTranslation(new Translation2d(
                            (mIsReversed ? -1.0 : 1.0) * (DriveConstants.kPathMinLookaheadDistance -
        ...
        DifferentialDrive.ChassisState adjusted_velocity = new DifferentialDrive.ChassisState();
        // Feedback on longitudinal error (distance).
        adjusted_velocity.linear = dynamics.chassis_velocity.linear + DriveConstants.kPathKX * Units.inches_to_meters
                (mError.getTranslation().x());
    ...
    public boolean hasTrajectory(){
        return mCurrentTrajectory != null;
    }
}
